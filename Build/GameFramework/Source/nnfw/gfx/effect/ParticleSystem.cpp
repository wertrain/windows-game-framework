...
bool ParticleSystem::Create(ID3D11Device* device, ID3D11DeviceContext* context, const wchar_t *fileName, const uint32_t instanceNum)
{
    NNFW_UNUSED(context);
...
bool ParticleSystem::Emit(const Vector4& pos)
{
    static int count = 0;
    Particle particle;
    particle.pos = pos;
    ...
    particle.velocity = Vector4(-0.01f + sRand.NextFloat() * 0.03f, sRand.NextFloat() * 0.1f, -0.01f + sRand.NextFloat() * 0.03f, 0);
    particle.flag = Particle::Flags::Alive;
    return mParticles.Enqueue(particle);
}

void ParticleSystem::EmitAll()
{
...
void ParticleSystem::Render(ID3D11DeviceContext* context)
{
    ...
    // インスタンシング描画
    {
        ...
        {
            Particle& particle = mParticles[index];
            if ((particle.flag & Particle::Flags::Alive) == 0) continue;
